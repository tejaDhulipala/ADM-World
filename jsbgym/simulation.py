...
class Simulation:
    ...
    def __setitem__(
        ...
        Warning: JSBSim will create new properties if the specified one does not exist.
        ...
        for p in self.jsbsim.get_property_catalog():
            if jsbsim_name in p:
                ...
                if "(R)" in p:
                    read_only = True
                    warnings.warn(f"Attempting to set a value to a read-only value, {jsbsim_name}")
                if "W" in p:
                    write_only = True
                break
        if not exists:
