...
altitude_agl_ft = BoundedProperty(
    "position/h-agl-ft", "altitude above ground level [ft]", -1400, 85000
)
angle_of_attack = BoundedProperty("aero/alpha-deg", "Angle of attack degrees", -90, 90)
...
oil_pressure_psi = Property(
    "propulsion/engine/oil-pressure-psi", "engine oil pressure [psi]"
)
oil_temperature_degF = Property(
    "propulsion/engine/oil-temperature-degF", "engine oil temperature [degF]"
)
...
propeller_rpm = Property("propulsion/engine/propeller-rpm", "Propellor RPM")
...
mixture_cmd = BoundedProperty(
    "fcs/mixture-cmd-norm", "engine mixture setting, normalised. 1 is full rich, 0 is idle cutoff.", 0.0, 1.0
)
...
pitch_trim = Property("fcs/pitch-trim-cmd-norm", "Pitch trim, degrees")
...
# initial conditions
initial_altitude_msl_ft = Property("ic/h-sl-ft", "initial altitude MSL [ft]")
initial_altitude_agl_ft = Property("ic/h-agl-ft", "initial altitude AGL [ft]")
...
ap_hold_altitude = Property("ap/altitude_hold", "Active when set to 1. Changes elevator to get to ap/altitude_setpoint")
ap_hold_heading = Property("ap/heading_hold", "Active when 1. Changes ailerons to ap/heading_setpoint")
ap_heading_setpoint = Property("ap/heading_setpoint", "Heading setpoint in compass heading")
ap_altitude_setpoint = Property("ap/altitude_setpoint", "Altitude setpoint in MSL feet")

# Forces
load_factor = Property("forces/load-factor", "Load factor")

# Accelerations
pdot_rad_sec2 = Property(
    "accelerations/pdot-rad_sec2", "body frame roll acceleration"
)
qdot_rad_sec2 = Property(
    "accelerations/qdot-rad_sec2", "body-frame pitch acceleration"
)
rdot_rad_sec2 = Property(
    "accelerations/rdot-rad_sec2", "body-frame yaw acceleration"
)
